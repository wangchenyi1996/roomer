<template>
  <div class="findHome" :class="fix ? 'fix-home' :''">
    ...
    <div class="room-around">
      <div class="room-list" v-for="(item,index) in roomlists" :key="index">
        <img src="../assets/imgs/findHome/home.png" v-if="index==8" />
        <Room :item="item" :index="index" v-else />
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.getRoomList();
    this.roomlists = this.roomList;
    this.lastRoomList = this.roomList;
    console.log(this.roomlists)
  },
  activated() {
    // if (this.lastRoomList.length !== this.roomList.length) {
      // this.lastRoomList = this.roomList;
      this.roomlists = this.roomList;
      this.getRoomList();
    // }
